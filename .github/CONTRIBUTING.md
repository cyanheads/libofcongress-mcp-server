...
Thanks for using `libofcongress-mcp-server`. Bugs, feature requests, and documentation gaps all belong in an issue — that's where they get read and picked up.
...
`libofcongress-mcp-server` is built on [@cyanheads/mcp-ts-core](https://github.com/cyanheads/mcp-ts-core), which handles transports, auth, config, logging, and telemetry. Sorting out which layer broke saves everyone a round-trip:
...
Don't open a public issue for a vulnerability. Report it privately through GitHub's **Security** tab → **Report a vulnerability**, or email [security@caseyjhand.com](mailto:security@caseyjhand.com).
